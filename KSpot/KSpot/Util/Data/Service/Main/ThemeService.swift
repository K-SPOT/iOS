...
import Foundation
struct ThemeService : GettableService {
    typealias NetworkData = ThemeVO
    static let shareInstance = ThemeService()
    func getThemeData(url : String, completion : @escaping (NetworkResult<Any>) -> Void){
        get(url) { (result) in
            switch result {
