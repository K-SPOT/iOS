...
import Foundation
struct SearchResultService : GettableService {
    typealias NetworkData = SearchResultVO
    static let shareInstance = SearchResultService()
    func getSearchResult(url : String, completion : @escaping (NetworkResult<Any>) -> Void){
        get(url) { (result) in
            switch result {
