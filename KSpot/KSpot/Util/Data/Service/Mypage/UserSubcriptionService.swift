...
import Foundation
struct UserSubcriptionService: GettableService {
    
    typealias NetworkData = UserSubcriptionVO
    static let shareInstance = UserSubcriptionService()
    func getSubscriptionList(url : String, completion : @escaping (NetworkResult<Any>) -> Void){
        get(url) { (result) in
            switch result {
