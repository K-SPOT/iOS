...
struct ChannelService: GettableService {
    
    typealias NetworkData = ChannelVO
    static let shareInstance = ChannelService()
    func getChannelList(url : String, completion : @escaping (NetworkResult<Any>) -> Void){
        get(url) { (result) in
            switch result {
