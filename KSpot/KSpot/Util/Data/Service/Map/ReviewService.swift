...
struct ReviewService: GettableService {
    
    typealias NetworkData = ReviewVO
    static let shareInstance = ReviewService()
    func getReviewData(url : String, completion : @escaping (NetworkResult<Any>) -> Void){
        get(url) { (result) in
            switch result {
